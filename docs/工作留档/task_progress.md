...
## 2026-09-15：ESP32 音频半声后持续无声排查与修复

- 用户报告唤醒后欢迎音只出现半声“Hi”，随后服务器回复继续正常生成但硬件无声。核验最新 10 个回复 WAV 均为 ESP 支持的 16kHz/单声道/16-bit PCM，时长约 2～7 秒、峰值和 RMS 非零，最大约 229KB，未超过固件 512KB 下载上限；内置欢迎音同样异常，因此问题范围收敛到 ESP I2S 输出、MAX98357A/扬声器或供电，而不是 New_ManBoo 合成格式。
- 代码确认 `i2s_channel_write()` 只保证数据复制到 DMA，旧实现随后立即 `i2s_channel_disable()`，会停止 BCLK/WS 并清空 TX 队列，存在截断尾部和反复停启功放的风险。修复为 TX DMA 自动清零、音频后等待完整 DMA 环排空、空闲保持数字静音，不再逐段禁用 I2S；重新启用时增加功放稳定等待，并把短写入视为失败。
- 固件版本更新为 `2026-09-15-audio-v1`，新增串口 `test audio`，可绕过网络和服务器直接播放内置欢迎音，区分 I2S 软件错误与功放/接线/供电故障。同步更新串口指南与结构地图。
- `platformio run` 已成功（RAM 48,604/327,680 字节，14.8%；Flash 1,478,103/2,048,000 字节，72.2%）。经用户明确授权，已成功烧录 COM7，写入 SHA 校验通过并自动复位；串口确认板上运行 `2026-09-15-audio-v1`、Wi-Fi connected、AI bridge ready、服务器地址为 `192.168.43.33:8000/voice/interact`。
- 已连续发送两次 `test audio`，两次均写入完整 37,632 字节并完成 110ms DMA 排空等待，没有 I2S 驱动报错；监视器随后已关闭并释放 COM7。软件日志只能证明数据送达 I2S，扬声器是否两次完整发声仍待用户听感确认；若日志成功但无声，应检查 MAX98357A 供电/共地/三根信号线和扬声器。

## 2026-09-15：服务启动指南增加多音色选择

- 用户已按原指南启动全部服务。本轮只读核验确认本机 11434、9880、8000 均在监听，8000 `/config` 为 `tts_backend=gpt_sovits`、参考音频已配置；未据此推断当前 9880 的具体权重，因为该接口不回显权重名。
- 已确认最新训练候选文件实际存在：GPT `New_ManBoo-e5.ckpt`、SoVITS `New_ManBoo_e4_s80.pth`；同实验的 `New_ManBoo_e2_s40.pth` 是较早检查点。最新候选仅代表最近导出的权重，尚未记录用户试听验收，不能写成质量已通过。
- 更新 `docs/项目指南/项目重启后服务启动指南.md`：将模型定义为“GPT 权重 + SoVITS 权重 + 参考音频 + 参考原文”的完整音色档案；补充已启动状态下热切换 New_ManBoo、使用匹配参考片段重启 8000、以后多模型登记/选择、默认档案恢复与故障判断。
- 本轮没有执行权重切换、没有重启当前服务、没有修改固件或服务器代码、没有烧录。文档中的 New_ManBoo 切换命令待用户实际执行和试听验证。

## 2026-09-15：三处本地项目集中（最新）

- 用户要求把Projects2026下两个项目与college资料实际集中。本轮保持唯一现行根目录`E:\Projects2026\Prp_voice_servo_unified`，已将`E:\Projects2026\Prp`移入`legacy\Prp_servo_arduino`，将`E:\college\毛绒治愈机器人PRP`移入`archive\PRP_原始项目资料`。旧两个顶层目录均已不存在，Projects2026下只剩统一项目目录；不是复制或快捷方式。
- 迁移清单：182＋1046＝1228文件，共386,801,862字节，源无reparse points，目标此前不存在。逐文件SHA256前后核对一致，没有覆盖或删除文件，迁移后只删除空的源目录。首次PowerShell整目录移动因Windows拒绝操作导致部分迁移，立即核对46项已迁、136项仍在原处、0缺失；随后按清单使用不覆盖的逐文件移动安全续迁，最终两组status均verified。
- 旧舵机Git保留原提交`9e0014c`、3项已修改文件与1项未跟踪工作区文件；迁移后git status一致，未代用户提交或重置旧仓库。原college中调研/PPT/清单/发票/旧语音源码/固件备份完整保留，不混入现行main/server，也未删除重复历史资料。
- 新增根`项目入口.md`和单根`PRP.code-workspace`；新增`tools/consolidate_project_folders.ps1`作为已执行的一次性迁移审计/续迁工具（需PowerShell7，不作日常启动脚本）。完整文件哈希清单在`.local_migration\20260915\files.json`，保留本地。更新README、文档索引、结构地图、工作记忆及AGENTS导航。
- `.gitignore`新增legacy/archive/.local_migration排除项；当前私有仓库保存迁移脚本、导航和文档，不自动上传原发票、旧固件/第三方源码或旧独立仓库。两份归档已在本地集中，但不是已经做了云备份；需要云备份时再按资料类别审查。
- 验证：迁移脚本语法检查通过，7项服务端/数据离线测试通过，统一工程`platformio run`成功（8.22秒，RAM14.8%、Flash72.2%）。本轮未改当前固件/服务器逻辑、未启动或重启服务、未烧录、未做硬件实测；旧Arduino工程未在新位置重新编译。
- 注意：旧快捷方式/最近打开项目和归档中工作区的历史跨目录引用可能失效，后续统一从根`PRP.code-workspace`进入。项目外GPT-SoVITS仍位于`E:\AI\GPTSoVITS\GPT-SoVITS-v2pro-20250604`，本轮没有搬软件安装目录。
...
## 2026-09-22：五舵机PLA骨架V0与硬件尺寸核对

- 用户确认四腿承重、躯干70×50×50可扩大、PLA可调填充率、成型空间200×200×200。读取硬件参数9张图片，记录SG90孔距27.8/孔径2.2、ESP32含天线63.389×27.94、PCA9685 60×25、MAX98357A 18×19/上孔距12.6、扬声器35×25等。电池新图3000mAh与订单2000mAh不一致；降压板新图CR-D4080P 60×30与旧订单小板不一致，仍待实物确认；未猜测未标孔径。
- 生成mechanical/v0：主框架150×94×65，可拆电池托、两层板托、两降压滑座、四套独立M4腿轴/轴套/36mm连杆、五路SG90安装位、尾臂、扬声器框和压条；未知麦克风/配电附件仅预留，不宣称所有充电/触摸模块已完成专用装配。整体包络220.7×166×104，32个分件各自均小于200mm。
- 完成V0几何阶段验证：32个有效单实体及闭合STL、5种小样闭合STL；静态打印件和硬件参考包络无>0.05mm³体积干涉；四腿±7.5°、±15°离散姿态无打印件干涉；STEP独立回读56个有效实体。未做连续扫掠、有限元、加载、疲劳、温升或实物装配；V0不得作为承重验收完成的定版。
- 已通过Windows应用控制在用户SolidWorks 2024打开STEP并保存PRP_V0_with_hardware.SLDASM；保留同目录STEP引用，没有原生草图特征历史，尚未创建运动配合。未再操作受阻的淘宝页面。
- CAD依赖隔离安装于用户缓存；所有生成任务已结束，无新常驻服务。未修改固件/服务器逻辑；按项目规则执行platformio run成功，RAM14.8%、Flash72.2%，未烧录、未开串口。现有无关工作区修改保留。
- 下一步：先核实电池/降压板SKU及高度、原装舵盘孔位、板卡排针高度；打印T01/T02/T04/T05，再做单腿无电试装与逐级加载；测整机质量/扭矩/电流后才能定承重和是否保留SG90。新硬件不匹配时修改V0，优先缩小大降压板预留。

## 2026-09-22：改为独立硬件草模交付

- 用户认为 V0 整体过大、连接复杂，明确改为自行设计骨架与连接；停止推进 V0 骨架方案，历史产物保留。
- 从已有 STEP 实体提取 SG90、ESP32-S3、PCA9685、电池、降压板、MAX98357A、扬声器七种草模；另放 INMP441 暂定占位，并提供展示总览。现行交付目录为 mechanical/硬件草模。
- 统一 mm、XY 外形中心原点与底面 Z=0；板卡相接包络合并为单实体。八个单件均通过有效性、体积保持与 STEP 回读检查，核验 JSON 已保存。只有 SG90 保留原有安装孔，其他零件多数为简化包络，不能当作精确孔位模板。
- README 标明电池/降压板型号冲突、暂定高度、麦克风未定尺寸，以及 SolidWorks STEP 导入方式。未进行本批单件的 SolidWorks 逐件导入或实物验收。
- 本阶段仅整理 CAD 产物与文档，未修改程序代码，未重跑固件构建、未烧录。提交仅包含本阶段文件和文档增量，保留其他工作区改动。

## 2026-09-22：论文示意图统一重绘

- 重绘论文中的图2-1四层总体架构、图3-1成品供电结构和图4-1语音交互数据流：统一微软雅黑、圆角框、留白和低饱和配色；所有连接改为水平或垂直的正交箭头，移除图片内部重复图题，避免箭头倾斜、标签压线和文字错位。
- 图中继续区分实现与验证状态：成品供电和五舵机整体仍标为待联调，连续对话新版仍标为已编译待烧录实测，PCA9685仅通道0有实际运动验证，不将计划或编译结果写成整机实测。
- 原正式版DOCX当时被Word占用，未强制关闭或覆盖；生成 `论文初稿/毛绒陪伴机器人语音与动作系统设计_正式版_图示优化版.docx` 作为完整优化稿。使用本机Word只读导出并逐页检查9页，封面、正文分页和三张图题均保持正常，无图片裁切、文字重叠或连线错位。
- 新增 `paper_tools/refresh_paper_figures.py`，可重复生成三张高分辨率PNG并替换DOCX内对应媒体；未修改固件或服务端代码，未烧录ESP32。
