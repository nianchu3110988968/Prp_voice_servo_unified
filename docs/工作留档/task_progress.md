...
- `platformio run` 已成功（RAM 48,604/327,680 字节，14.8%；Flash 1,478,103/2,048,000 字节，72.2%）。尚未烧录或硬件实测；COM7 被用户当前的串口监视器占用，未强制关闭。只有烧录新版后才能用 `test audio` 验证，烧录必须再次获得用户明确确认。
